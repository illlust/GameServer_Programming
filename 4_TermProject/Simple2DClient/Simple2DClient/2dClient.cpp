...
void ProcessPacket(char* ptr)
{
	...
	case S2C_ENTER:
	{
		...
		if (id == g_myid) {
			...
			{
				if(my_packet->npcCharacterType == NPC_PEACE && my_packet->npcMoveType == NPC_FIX) //빨강
					npcs[id] = OBJECT{ *pacman_pieces, 0, 31 * 0, 31, 31 };
				else if (my_packet->npcCharacterType == NPC_PEACE && my_packet->npcMoveType == NPC_RANDOM_MOVE) //핑크
					npcs[id] = OBJECT{ *pacman_pieces, 0, 31 * 1, 31, 31 };
				else if (my_packet->npcCharacterType == NPC_WAR && my_packet->npcMoveType == NPC_FIX) //파랑
